...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def delete_message(self, chat_id: str, message_id: str) -> bool:
        """Delete one exact message selected by trusted gateway bookkeeping.

        This is an internal adapter primitive, not a Discord/model action. Callers
        must pass IDs returned by this adapter for the active turn (for example,
        the post-final transient-progress cleanup path in ``gateway.run``).
        """
        if not self._client:
            return False
        try:
